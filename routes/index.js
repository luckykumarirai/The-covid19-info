...
router.get('/', function(req, res, next) {
  let countrytotal="https://coronavirus-19-api.herokuapp.com/countries/india";
  fetch(countrytotal, settings).then(res => res.json()).then((totaldata) => {
    res.render('country', { title: title,countrytotal:totaldata,country:"India",useremail:req.session.user });
  });
});

router.get('/indiastatewise', function(req, res, next) {
  let stateurl="https://api.covid19india.org/v4/min/data.min.json";
...
router.get('/helpline', function(req, res, next) {
  let stateurl="https://api.rootnet.in/covid19-in/contacts";
  fetch(stateurl, settings).then(res => res.json()).then((data) => {
    res.render('helpline', { title: title,indiahelpline:data.data.contacts,useremail:req.session.user,country:req.session.country });
  });
});

router.get('/cowin', function(req, res, next) {
  res.render('cowin', { title: title,centers:"",certificatelink:"",useremail:req.session.user,country:req.session.country });
});

router.post('/cowinpin', function(req, res, next) {
  var pin=req.body.pincode;
  res.render('cowin', { title: title,pin:pin,certificatelink:"",useremail:req.session.user,country:req.session.country });
});

router.post('/cowincertificate', function(req, res, next) {
  var bid=req.body.id;
  var link="https://cdn-api.co-vin.in/api/v2/registration/certificate/public/download?beneficiary_reference_id="+bid;
  res.render('cowin', { title: title,centers:"",certificatelink:link,useremail:req.session.user,country:req.session.country });
});
